...
const conString = "postgres://bankbuser:password@localhost:5432/bankb";
...
app.post('/userdet', function(request, response) {
    console.log("userdet", request.body);
...
app.post('/payment', function(request, response) {
    ...
    clientA.query('SELECT * from users where friendlyid = $1 and domain = $2', [senderName, senderDomain], (error, results) => {
        if (error) {
        ...
        if (results && results.rows[0]) {
                ...
                requestObj.post({
                    ...
                    if (err || res.statusCode !== 200) {
                        ...
                        if (results) {
                            ...
                            clientA.query('UPDATE users set balance = $1 where friendlyid = $2', [balance, senderName], (error, results) => {
            ...
            response.statusMessage = "No matching user"
            response.status(500).send("Could not find any matching user")
        }
    })
});
